...
def build_argparser():
    p = argparse.ArgumentParser(prog="exp01", description="Exp01 S2-paper Δ≤5 (skeleton + naive)")
...
def naive_last_value_per_fold(df: pd.DataFrame, date_col: str, y_col: str):
    """Para cada fold LODO, prevê o **último valor do treino** (média da última data de treino)."""
    folds = make_lodo_splits(df, date_col)
    rows = []
    for f in folds:
        train = df.iloc[f.train_idx]
        test  = df.iloc[f.test_idx]
        # última data disponível no treino
        last_d = pd.to_datetime(train[date_col]).max()
        # valor-naive = média do y na última data de treino
...
def main():
    ...
    # rodar baseline Naïve e salvar métricas por fold + agregados
