...
def main():
    ...
    # ----------------------
    # Features estáveis via XGB (CP) - baseadas no arquivo de estabilidade
    # ----------------------
    ...
    # Colunas de clima que NÃO queremos em 'noclim'
    clima_cols = [
    ...
    features = [f for f in stable if f not in clima_cols]
    print("[INFO] Ignorando colunas de clima (noclim):", clima_cols)
    ...
    print(f"\n[GLOBAL OOF - GB_D7_CP_NOCLIM_FS] R2={r2_global:.3f} | RMSE={rmse_global:.3f} | MAE={mae_global:.3f}")
    ...
    metrics_path = out_dir / "baseline_experimento_UFMS_D7_CP_noclim_gb_fs_metrics.csv"
    oof_path = out_dir / "baseline_experimento_UFMS_D7_CP_noclim_gb_fs_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (GB D7_CP_noclim_gb_fs) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (GB D7_CP_noclim_gb_fs) salvas em: {oof_path}")
