...
# Notas: Script de experimento LODO para CP (proteína bruta), usando XGBoost (árvore), janela Δ5 dias (D5), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def run_lodo():
    ...
    print("[INFO] Iniciando LODO por Date (XGB D5, noclim)...")
    ...
    for i, d in enumerate(unique_dates, start=1):
        ...
        # Modelo XGB simples de baseline
        model = XGBRegressor(
            n_estimators=400,
            learning_rate=0.05,
            max_depth=3,
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_xgb_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_xgb_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (XGB D5 noclim) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (XGB D5 noclim) salvas em: {oof_path}")
