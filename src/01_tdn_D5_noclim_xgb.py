...
# Notas: Script de experimento LODO para TDN_based_ADF (digestibilidade estrutural), usando XGBoost (árvore), janela Δ5 dias (D5), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def run_lodo():
    ...
    print(
        f"\n[GLOBAL OOF - XGB_D5_TDN_NOCLIM_NOADF] "
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_TDN_noclim_xgb_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_TDN_noclim_xgb_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (XGB D5 TDN noclim, sem ADF/TDN_NDF/CP) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (XGB D5 TDN noclim, sem ADF/TDN_NDF/CP) salvas em: {oof_path}")
