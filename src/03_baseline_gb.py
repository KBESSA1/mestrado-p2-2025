...
def main():
    ...
    for fold in make_lodo_splits(df, args.date_col):
        ...
        rows.append({"fold_date": dtag, "model": "gb", **m})

        pd.DataFrame({"fold_date": dtag, "y_true": yte, "y_pred": yhat}) \
          .to_csv(os.path.join(preds_dir, f"preds_gb_{args.target_col}_{dtag}.csv"), index=False)
        scatter_plot(yte, yhat, f"GB | {args.target_col} | {dtag}",
                     os.path.join(plots_dir, f"scatter_gb_{args.target_col}_{dtag}.png"))
    ...
    rows.append({"fold_date": "__mean__", "model": "gb", "r2": r2m, "rmse": rmsem, "mae": maem})
    pd.DataFrame(rows).to_csv(out_csv, index=False)

    if preds_all:
        big = pd.concat(preds_all, ignore_index=True)
        big.to_csv(os.path.join(preds_dir, f"preds_gb_{args.target_col}_ALL.csv"), index=False)
        scatter_plot(big["y_true"], big["y_pred"], f"GB | {args.target_col} | TODOS OS FOLDS",
                     os.path.join(plots_dir, f"scatter_gb_{args.target_col}_ALL.png"))
