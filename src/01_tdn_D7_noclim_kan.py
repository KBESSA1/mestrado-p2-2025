...
# Notas: Script de experimento LODO para TDN_based_ADF (digestibilidade estrutural), usando Kolmogorov-Arnold Network (KAN), janela Δ7 dias (D7), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def run_lodo_kan():
    ...
    print(
        f"\n[GLOBAL OOF - KAN_D7_TDN_NOCLIM_NOADF] "
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D7_TDN_noclim_kan_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D7_TDN_noclim_kan_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (KAN D7 TDN noclim, sem ADF/TDN_NDF/CP) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (KAN D7 TDN noclim, sem ADF/TDN_NDF/CP) salvas em: {oof_path}")
