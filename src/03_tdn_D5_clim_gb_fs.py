...
def main():
    ...
    print("[INFO] Features estáveis (antes de remover leak):", stable)
    ...
    print(f"\n[GLOBAL OOF - GB_D5_TDN_based_ADF_CLIM_FS] R2={r2_global:.3f} | RMSE={rmse_global:.3f} | MAE={mae_global:.3f}")
    ...
    metrics_path = out_dir / "baseline_experimento_UFMS_D5_TDN_based_ADF_clim_gb_fs_metrics.csv"
    oof_path = out_dir / "baseline_experimento_UFMS_D5_TDN_based_ADF_clim_gb_fs_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (GB D5_TDN_based_ADF_clim_gb_fs) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (GB D5_TDN_based_ADF_clim_gb_fs) salvas em: {oof_path}")
