...
# Notas: Script de experimento LODO para CP (proteína bruta), usando MLP densa (rede neural), janela Δ7 dias (D7), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data(use_climate: bool = False):
    ...
    if not use_climate:
        # Ignorar colunas de clima se existirem
...
def run_lodo(use_climate: bool = False):
    ...
    for i, d in enumerate(unique_dates, start=1):
        ...
        rmse = float(np.sqrt(mse))  # <<< AQUI substitui o squared=False
    ...
    metrics_path = OUT_DIR / f"baseline_experimento_UFMS_D7_CP_{suffix}_mlp_metrics.csv"
    oof_path = OUT_DIR / f"baseline_experimento_UFMS_D7_CP_{suffix}_mlp_oof.csv"
...
if __name__ == "__main__":
    # Aqui é o script D7 CP *sem clima*
