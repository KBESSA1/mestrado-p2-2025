...
# Notas: Script de experimento LODO para CP (proteína bruta), usando Ridge Regression, janela Δ5 dias (D5), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data():
    ...
    # Também tiramos IDs numéricos, se existirem
...
def run_lodo():
    ...
    print("[INFO] Iniciando LODO por Date (Ridge D5, noclim)...")
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_ridge_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_ridge_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (Ridge D5 noclim) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (Ridge D5 noclim) salvas em: {oof_path}")
