...
def main():
    ...
    # ----------------------
    # Features estáveis via XGB (TDN) - base COM clima
    # ----------------------
    ...
    print("[INFO] Features estáveis (antes de remover leak/clima):", stable)

    # Colunas de leak + clima (cenário NOCLIM)
    leak_cols = ["ADF", "TDN_based_NDF", "CP"]
    clima_cols = [
        "TEMP_MAX", "TEMP_MIN", "RAIN", "RAD_SOL",
        "Radiative_Dif_AVG", "Radiative_Direct_AVG",
        "Longwave_Rad_AVG", "TP_SFC_AVG", "PRES_ATM",
        "EVAPOT", "WIND_SPD", "Wind_Dir", "HUM_REL", "PPFD"
    ]

    drop_cols = set(leak_cols + clima_cols)

    features = [f for f in stable if f not in drop_cols]
    print("[INFO] Ignorando colunas de leak/clima:", [c for c in stable if c in drop_cols])
    ...
    print(f"[INFO] Shape de X: ( {X.shape[0]}, {X.shape[1]} ), y: {y.shape}")
    ...
    print(f"\n[GLOBAL OOF - GB_D7_TDN_based_ADF_NOCLIM_FS] R2={r2_global:.3f} | RMSE={rmse_global:.3f} | MAE={mae_global:.3f}")
    ...
    metrics_path = out_dir / "baseline_experimento_UFMS_D7_TDN_based_ADF_noclim_gb_fs_metrics.csv"
    oof_path = out_dir / "baseline_experimento_UFMS_D7_TDN_based_ADF_noclim_gb_fs_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (GB D7_TDN_based_ADF_noclim_gb_fs) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (GB D7_TDN_based_ADF_noclim_gb_fs) salvas em: {oof_path}")
