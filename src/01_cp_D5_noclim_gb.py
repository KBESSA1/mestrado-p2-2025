...
# Notas: Script de experimento LODO para CP (proteína bruta), usando Gradient Boosting (árvore), janela Δ5 dias (D5), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
DELTA_MAX_DAYS = 5  # D5 de verdade
...
def prepare_data():
    ...
    # Features numéricas, ignorando clima (noclim)
    ...
    drop_cols = [TARGET_COL, "Delta_days"] + clima_presentes
...
def run_lodo():
    ...
    print("[INFO] Iniciando LODO por Date (GB D5, noclim)...")
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_gb_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_gb_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (GB D5 noclim) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (GB D5 noclim) salvas em: {oof_path}")
