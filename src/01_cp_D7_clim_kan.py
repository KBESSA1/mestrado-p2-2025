...
# Notas: Script de experimento LODO para CP (proteína bruta), usando Kolmogorov-Arnold Network (KAN), janela Δ7 dias (D7), com clima acoplado ao S2. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data():
    ...
    # Features numéricas, COM clima (CLIM): tira só o alvo
    num_cols = df.select_dtypes(include=["number"]).columns.tolist()
    feature_cols = [c for c in num_cols if c != TARGET_COL]

    print("[INFO] Colunas de clima usadas (D7 CLIM):", CLIMATE_COLS)
...
def run_lodo_kan():
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D7_CP_clim_kan_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D7_CP_clim_kan_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (KAN D7 CP CLIM) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (KAN D7 CP CLIM) salvas em: {oof_path}")
