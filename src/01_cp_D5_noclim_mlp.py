...
# Notas: Script de experimento LODO para CP (proteína bruta), usando MLP densa (rede neural), janela Δ5 dias (D5), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def run_lodo(use_climate: bool = False):
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_mlp_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_mlp_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (MLP D5 noclim) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (MLP D5 noclim) salvas em: {oof_path}")


if __name__ == "__main__":
    run_lodo(use_climate=False)
