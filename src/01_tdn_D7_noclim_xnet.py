...
# Notas: Script de experimento LODO para TDN_based_ADF (digestibilidade estrutural), usando XNet (Cauchy network), janela Δ7 dias (D7), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data():
    ...
    # Remover alvo, Delta_days, leaks e clima (noclim)
    base_drop = [TARGET_COL, "Delta_days"] + LEAK_COLS + CLIMATE_COLS
...
def run_lodo_xnet():
    ...
    print(
        f"\n[GLOBAL OOF - XNET_D7_TDN_NOCLIM_NOADF] "
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D7_TDN_noclim_xnet_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D7_TDN_noclim_xnet_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(
        f"\n[OK] Métricas por fold + GLOBAL (XNet D7 TDN noclim, sem ADF/TDN_NDF/CP) salvas em: {metrics_path}"
    )
    print(
        f"[OK] Predições OOF (XNet D7 TDN noclim, sem ADF/TDN_NDF/CP) salvas em: {oof_path}"
    )
