...
# Notas: Script de experimento LODO para CP (proteína bruta), usando Gradient Boosting (árvore), janela Δ5 dias (D5), com clima acoplado ao S2. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def run_lodo():
    ...
    print("[INFO] Iniciando LODO por Date (GB D5, CLIM)...")
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_clim_gb_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_clim_gb_oof.csv"

    print(f"[OK] Métricas por fold + GLOBAL (GB D5 CLIM) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (GB D5 CLIM) salvas em: {oof_path}")
