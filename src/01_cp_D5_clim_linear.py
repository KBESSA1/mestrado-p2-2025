...
# Notas: Script de experimento LODO para CP (proteína bruta), usando Regressão Linear, janela Δ5 dias (D5), com clima acoplado ao S2. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data():
    ...
    # Remover alvo e Delta_days das features
...
def run_lodo():
    ...
    print("[INFO] Iniciando LODO por Date (Linear D5, CLIM)...")
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_clim_linear_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_clim_linear_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (Linear D5 CLIM) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (Linear D5 CLIM) salvas em: {oof_path}")
