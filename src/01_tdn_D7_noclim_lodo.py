...
# Notas: Script de experimento LODO para TDN_based_ADF (digestibilidade estrutural), usando lodo, janela Δ7 dias (D7), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
CSV_PATH = Path("/workspace/data/data_raw/Complete_DataSet.csv")  # ajuste se precisar
...
SUFFIX = "D7_TDN_noclim_linear"  # vai no nome dos CSVs
...
class Winsorizer(BaseEstimator, TransformerMixin):
    """Winsoriza colunas numéricas por quantis (1% e 99%)."""
...
# Remover colunas de clima (cenário NOCLIM)
...
drop_cols = [TARGET_COL, DATE_COL, SAT_COL, "Delta_days"]
drop_cols = [c for c in drop_cols if c in df.columns]

num_cols = df.select_dtypes(include=[np.number]).columns.difference(drop_cols)

if len(num_cols) == 0:
    raise RuntimeError("Nenhuma coluna numérica de feature encontrada depois dos drops.")

print(f"[INFO] Nº de features numéricas: {len(num_cols)}")
print(f"[INFO] Algumas features: {list(num_cols[:10])}")
...
print("[INFO] Iniciando LODO por Date...")
...
print("\n[GLOBAL OOF] R2={:.3f} | RMSE={:.3f} | MAE={:.3f}".format(r2_global, rmse_global, mae_global))
...
print(f"\n[OK] Métricas por fold + GLOBAL salvas em: {metrics_path}")
print(f"[OK] Predições OOF salvas em: {oof_path}")
