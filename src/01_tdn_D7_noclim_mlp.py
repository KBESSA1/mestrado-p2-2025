...
# Notas: Script de experimento LODO para TDN_based_ADF (digestibilidade estrutural), usando MLP densa (rede neural), janela Δ7 dias (D7), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data():
    ...
    # Identificar clima e leaks presentes
    ...
    # Montar lista de colunas a remover do X
...
def run_lodo(use_cuda: bool = True):
    ...
    print(
        f"\n[GLOBAL OOF - MLP_D7_TDN_NOCLIM_NOADF] "
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D7_TDN_noclim_mlp_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D7_TDN_noclim_mlp_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (MLP D7 TDN noclim, sem ADF/TDN_NDF/CP) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (MLP D7 TDN noclim, sem ADF/TDN_NDF/CP) salvas em: {oof_path}")
