...
# Notas: Script de experimento LODO para TDN_based_ADF (digestibilidade estrutural), usando Ridge Regression, janela Δ7 dias (D7), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data():
    ...
    # NÃO deixar alvo, Delta_days, clima nem leak entrarem em X
    drop_cols = [TARGET_COL, "Delta_days"] + clima_presentes + leak_presentes
...
def run_lodo_ridge():
    ...
    for i, d in enumerate(unique_dates, start=1):
        ...
        # Escalonamento com info só do treino
    ...
    print(
        f"\n[GLOBAL OOF - RIDGE_D7_TDN_NOCLIM_NOADF] "
    ...
    # Mantemos o mesmo padrão de nome de arquivo para não poluir demais.
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D7_TDN_noclim_ridge_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D7_TDN_noclim_ridge_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (Ridge D7 TDN noclim, sem ADF/TDN_NDF/CP) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (Ridge D7 TDN noclim, sem ADF/TDN_NDF/CP) salvas em: {oof_path}")
