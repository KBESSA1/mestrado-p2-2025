...
# Notas: Script de experimento LODO para TDN_based_ADF (digestibilidade estrutural), usando XNet (Cauchy network), janela Δ5 dias (D5), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
DELTA_MAX_DAYS = 5  # D5
USE_CLIMATE = False  # NOCILM

# colunas que entregam o TDN e NÃO podem entrar no X
...
def prepare_data():
    ...
    # Remove alvo, Delta_days e leaks
    ...
    if USE_CLIMATE:
        ...
        # nada a remover aqui; já estão dentro de numeric_cols
    else:
        print(f"[INFO] Ignorando colunas de clima (noclim): {CLIMATE_COLS}")
        feature_cols = [c for c in feature_cols if c not in CLIMATE_COLS]

    print(
        f"[INFO] Nº de features numéricas ({'com clima' if USE_CLIMATE else 'sem clima, sem leak'}): "
        f"{len(feature_cols)}"
    )
...
def run_lodo_xnet():
    ...
    print(
        f"\n[GLOBAL OOF - XNET_D5_TDN_NOCLIM_NOADF] "
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_TDN_noclim_xnet_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_TDN_noclim_xnet_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(
        f"\n[OK] Métricas por fold + GLOBAL (XNet D5 TDN noclim, sem ADF/TDN_NDF/CP) salvas em: {metrics_path}"
    )
    print(
        f"[OK] Predições OOF (XNet D5 TDN noclim, sem ADF/TDN_NDF/CP) salvas em: {oof_path}"
    )
