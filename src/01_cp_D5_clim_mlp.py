...
# Notas: Script de experimento LODO para CP (proteína bruta), usando MLP densa (rede neural), janela Δ5 dias (D5), com clima acoplado ao S2. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def run_lodo():
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_clim_mlp_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_clim_mlp_oof.csv"

    print(f"[OK] Métricas por fold + GLOBAL (MLP D5 CLIM) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (MLP D5 CLIM) salvas em: {oof_path}")
