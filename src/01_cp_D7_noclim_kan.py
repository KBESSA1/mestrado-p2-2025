...
# Notas: Script de experimento LODO para CP (proteína bruta), usando Kolmogorov-Arnold Network (KAN), janela Δ7 dias (D7), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data():
    ...
    # Features numéricas, SEM clima (noclim)
...
def run_lodo_kan():
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D7_CP_noclim_kan_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D7_CP_noclim_kan_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (KAN D7 CP noclim) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (KAN D7 CP noclim) salvas em: {oof_path}")
