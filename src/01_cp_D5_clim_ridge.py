...
# Notas: Script de experimento LODO para CP (proteína bruta), usando Ridge Regression, janela Δ5 dias (D5), com clima acoplado ao S2. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data():
    ...
    # Remover alvo e Delta_days das features (clima permanece como feature)
...
def run_lodo():
    ...
    print("[INFO] Iniciando LODO por Date (Ridge D5, CLIM)...")
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_clim_ridge_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_clim_ridge_oof.csv"

    print(f"[OK] Métricas por fold + GLOBAL (Ridge D5 CLIM) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (Ridge D5 CLIM) salvas em: {oof_path}")
