...
def main():
    ...
    # ----------------------
    # Features estáveis via XGB (TDN) - COM clima
    # ----------------------
    ...
    print("[INFO] Features estáveis (COM clima, antes de remover leak):", stable)

    leak_cols = ["ADF", "TDN_based_NDF", "CP"]
    drop_cols = set(leak_cols)

    features = [f for f in stable if f not in drop_cols]
    print("[INFO] Ignorando colunas de leak:", [c for c in stable if c in drop_cols])
    ...
    print(f"\n[GLOBAL OOF - GB_D7_TDN_based_ADF_CLIM_FS] R2={r2_global:.3f} | RMSE={rmse_global:.3f} | MAE={mae_global:.3f}")
    ...
    metrics_path = out_dir / "baseline_experimento_UFMS_D7_TDN_based_ADF_clim_gb_fs_metrics.csv"
    oof_path = out_dir / "baseline_experimento_UFMS_D7_TDN_based_ADF_clim_gb_fs_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (GB D7_TDN_based_ADF_clim_gb_fs) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (GB D7_TDN_based_ADF_clim_gb_fs) salvas em: {oof_path}")
