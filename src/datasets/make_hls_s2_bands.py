...
#!/usr/bin/env python3
"""
make_hls_s2_bands.py — Gera datasets HLS-S2 (bands-only) nos modos RAW, D5 e D7.

Nesta etapa, priorizamos destravar o RAW com QA correta e alinhamento perfeito:
- Coleção: Planetary Computer STAC, "hls2-s30" (Sentinel-2 no HLS v2)
- QA: Fmask como bitmask (excluir nuvem e sombra; água permitida)
- Buffer: 30 m (inalterado)
- RAW: escolhe melhor cena e calcula as bandas, realizando QA por banda (CRS da banda)

D5/D7 permanecem com agregação original; se necessário, aplicamos o mesmo tratamento por banda depois.
...
HLS_COLLECTION = "hls2-s30"  # CORRETO NA PC

# ----------------------------
# Estruturas e utilitários geo
# ----------------------------
@dataclass
class SceneQC:
    item_id: str
...
# QA (bitmask) — excluir nuvem/sombra; água permitida
def read_fmask_window(fmask_href: str, geom: dict) -> Tuple[np.ndarray, float, int]:
    with rasterio.open(fmask_href) as src:
        ...
        use = valid & (~cloudish)             # water (bit5) permitido
        total = int(valid.sum())
        cloud_frac = float(cloudish.sum()) / float(total) if total > 0 else 1.0
        return use, cloud_frac, total

def read_band_stat(href: str, geom: dict, use_mask: np.ndarray, prefer_median: bool = False) -> Tuple[Optional[float], int]:
...
# STAC helpers
# ----------------------------
def stac_search_items(lon: float, lat: float, date: pd.Timestamp, days_window: int, limit: int) -> List:
    ...
    # usar iterador moderno
    return list(search.items())

def pick_best_raw(items, target_date: pd.Timestamp, lon: float, lat: float, buffer_m: float) -> Tuple[Optional[SceneQC], Optional[dict], Optional[dict]]:
    best: Optional[SceneQC] = None
    best_assets: Optional[dict] = None
    best_geom: Optional[dict] = None
    for it in items:
        dt = pd.to_datetime(it.datetime).tz_localize(None)
        abs_days = int(abs((dt.date() - target_date.date()).days))
        signed = pc.sign(it)  # URLs com token
        fm = signed.assets.get("Fmask") or signed.assets.get("fmask")
        if fm is None:
            continue
        with rasterio.open(fm.href) as src_fm:
            geom, _ = reproject_point_buffer(lon, lat, buffer_m, src_fm.crs.to_string())
        use_mask, cloud_frac, n_valid_total = read_fmask_window(fm.href, geom)
        # se a janela inteira é nodata, ignore a cena
        if n_valid_total == 0:
            continue
        qc = SceneQC(
            item_id=signed.id,
...
def aggregate_window(items, lon: float, lat: float, buffer_m: float, prefer_median: bool = True) -> Tuple[Dict[str, float], List[SceneQC]]:
    per_band_vals: Dict[str, List[float]] = {v: [] for v in BAND_MAP.values()}
    qcs: List[SceneQC] = []
    for it in items:
        signed = pc.sign(it)
        fm = signed.assets.get("Fmask") or signed.assets.get("fmask")
        ...
        if n_valid_total == 0 or (use_mask.sum() == 0):
            continue
        dt = pd.to_datetime(it.datetime).tz_localize(None)
        qcs.append(SceneQC(item_id=signed.id, scene_datetime=dt, abs_days=0, cloud_frac_buffer=cloud_frac, n_valid_total=n_valid_total))
        for b, out_name in BAND_MAP.items():
            a = signed.assets.get(b)
            if a is None:
                continue
            val, nvalid = read_band_stat(a.href, geom, use_mask, prefer_median=prefer_median)
            if val is not None:
...
# ----------------------------
# Núcleo por linha (override RAW com QA por banda)
# ----------------------------
def process_row_mode(row: pd.Series, mode: str, buffer_m: float, limit: int):
    ...
    items = stac_search_items(lon, lat, date, days_window=15, limit=limit)
    if not items:
        return None, {"SampleID": sid, "mode": mode, "has_item": 0}

    if mode == "raw":
        best, assets, _ = pick_best_raw(items, date, lon, lat, buffer_m)
        if best is None or assets is None:
            return None, {"SampleID": sid, "mode": mode, "has_item": 0}
        fmask_href = assets.get("Fmask") or assets.get("fmask")
        ...
        for b, out_name in BAND_MAP.items():
            band_href = assets.get(b)
            if band_href is None:
                out[out_name] = np.nan
                continue
            # geom no CRS da banda
            with rasterio.open(band_href) as band_src:
                geom_band, _ = reproject_point_buffer(lon, lat, buffer_m, band_src.crs.to_string())
            # QA na mesma grade
            use_mask, cloud_frac_local, n_valid_total_local = read_fmask_window(fmask_href, geom_band)
            if n_valid_total_local == 0 or use_mask.sum() == 0:
                out[out_name] = np.nan
                continue
            # estatística da banda
            val, nvalid = read_band_stat(band_href, geom_band, use_mask, prefer_median=False)
            if val is None or nvalid == 0:
                out[out_name] = np.nan
            else:
                out[out_name] = val
                n_valid_any += nvalid
        ...
        return (out if n_valid_any > 0 else None), qc

    # D5/D7: manter agregação existente (sem per-banda por enquanto)
...
def main():
    ...
    if "SampleID" not in df.columns:
        sid_cand = next((c for c in ["Sample","Id","ID","id"] if c in df.columns), None)
        if sid_cand:
    ...
    # Sempre escreva QC (mesmo se 0 linhas válidas) para diagnóstico
    ...
    if len(out_rows) == 0:
        with open(qc_md, "w", encoding="utf-8") as f:
            ...
            f.write(f"Total UFMS: {len(df)}\n")
            f.write(f"Com bandas válidas: 0 (0.0%)\n\n")
            vc = df_qc.get("has_item", pd.Series(dtype='float')).value_counts(dropna=False).to_dict()
            f.write(f"has_item counts: {vc}\n")
        raise SystemExit("Sem linhas válidas. Revise máscara/CRS/buffer (QC salvo).")
        ...
        if args.mode == "raw":
            ...
            if "n_scenes_window" in df_qc.columns:
                ...
                for k in ["count","50%","75%","90%","max"]:
                    if k in desc and pd.notna(desc[k]):
                        f.write(f"{k}: {desc[k]}\n")
        f.write("\n## Observações\n\n- QA: Fmask (bitmask) — nuvem/sombra removidos; água mantida.\n- Buffer: 30 m.\n- RAW: QA por banda no CRS da banda para alinhamento robusto.\n")
