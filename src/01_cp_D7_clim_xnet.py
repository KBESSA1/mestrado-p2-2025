...
# Notas: Script de experimento LODO para CP (proteína bruta), usando XNet (Cauchy network), janela Δ7 dias (D7), com clima acoplado ao S2. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def run_lodo_xnet():
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D7_CP_clim_xnet_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D7_CP_clim_xnet_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(
        f"\n[OK] Métricas por fold + GLOBAL (XNet D7 CP CLIM) salvas em: {metrics_path}"
    )
    print(
        f"[OK] Predições OOF (XNet D7 CP CLIM) salvas em: {oof_path}"
    )
