...
# Notas: Script de experimento LODO para CP (proteína bruta), usando baseline ingênuo (persistência), janela Δ5 dias (D5), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data():
    ...
    # Preparar vetores para LODO
    y = df[TARGET_COL].to_numpy(dtype=float)
    dates = df["Date"].values
    unique_dates = np.unique(dates)
    print(f"[INFO] Nº de datas (folds LODO): {len(unique_dates)}")

    return df, y, dates, unique_dates


def run_lodo_naive():
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    df, y, dates, unique_dates = prepare_data()
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_naive_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_naive_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (Naive D5) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (Naive D5) salvas em: {oof_path}")
