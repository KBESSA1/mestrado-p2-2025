...
# Notas: Script de experimento LODO para TDN_based_ADF (digestibilidade estrutural), usando baseline ingênuo (persistência), janela Δ5 dias (D5), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def run_lodo():
    ...
    for i, d in enumerate(unique_dates, start=1):
        ...
        y_test_fold = y_test
        y_hat_fold = y_hat
        for dt, yt, yp in zip(fold_dates, y_test_fold, y_hat_fold):
            oof_rows.append(
                {
    ...
    print(
        f"\n[GLOBAL OOF - NAIVE_D5_TDN_NOCLIM] "
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_TDN_noclim_naive_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_TDN_noclim_naive_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (Naive D5 TDN noclim) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (Naive D5 TDN noclim) salvas em: {oof_path}")
