...
# Notas: Script de experimento LODO para CP (proteína bruta), usando XNet (Cauchy network), janela Δ5 dias (D5), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
DELTA_MAX_DAYS = 5  # D5
USE_CLIMATE = False  # este script é NOCLIM
...
class CauchyActivation(nn.Module):
    ...
    def forward(self, x):
        # x: (batch, dim)
        d = self.softplus(self.d_raw) + self.eps  # garante d > 0
        denom = x * x + d * d  # broadcast em dim
...
def prepare_data():
    ...
    # Remove o alvo e o Delta_days das features
    feature_cols = [c for c in numeric_cols if c not in [TARGET_COL, "Delta_days"]]

    # Opcional: remover colunas de clima (NOCLIM)
    if not USE_CLIMATE:
...
def run_lodo_xnet():
    ...
    for i, d in enumerate(dates_unique, start=1):
        ...
        for epoch in range(max_epochs):
            ...
            # Opcional: log leve
            # if (epoch + 1) % 100 == 0:
            #     print(f"  epoch {epoch+1:03d} | train_loss={epoch_loss:.4f}")
    ...
    metrics_df.loc[len(metrics_df)] = {
        "fold": "GLOBAL",
        "date": pd.NaT,
        "n_train": int(df.shape[0] - oof_df.shape[0]),  # não faz muito sentido, mas mantemos
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_xnet_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_xnet_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(
        f"\n[OK] Métricas por fold + GLOBAL (XNet D5 CP noclim) salvas em: {metrics_path}"
    )
    print(
        f"[OK] Predições OOF (XNet D5 CP noclim) salvas em: {oof_path}"
    )
