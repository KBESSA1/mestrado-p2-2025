...
# Notas: Script de experimento LODO para CP (proteína bruta), usando baseline ingênuo (persistência), janela Δ7 dias (D7), com clima acoplado ao S2. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def main():
    ...
    metrics_path = os.path.join(
        OUT_DIR,
        "baseline_experimento_UFMS_D7_CP_clim_naive_metrics.csv",
    )
    ...
    oof_path = os.path.join(
        OUT_DIR,
        "baseline_experimento_UFMS_D7_CP_clim_naive_oof.csv",
    )
    oof_df = pd.DataFrame({
