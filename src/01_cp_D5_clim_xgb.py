...
# Notas: Script de experimento LODO para CP (proteína bruta), usando XGBoost (árvore), janela Δ5 dias (D5), com clima acoplado ao S2. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data():
    ...
    if clima_presentes:
        print(f"[INFO] Colunas de clima usadas (D5 CLIM): {clima_presentes}")
    else:
        print("[INFO] Nenhuma coluna de clima encontrada, seguindo sem clima explícito.")
        clima_presentes = []

    # Remover alvo e Delta_days das features (mantém clima)
...
def run_lodo():
    ...
    print("[INFO] Iniciando LODO por Date (XGB D5, CLIM)...")
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_clim_xgb_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_clim_xgb_oof.csv"

    print(f"[OK] Métricas por fold + GLOBAL (XGB D5 CLIM) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (XGB D5 CLIM) salvas em: {oof_path}")
