...
# Notas: Script de experimento LODO para TDN_based_ADF (digestibilidade estrutural), usando Ridge Regression, janela Δ5 dias (D5), com clima acoplado ao S2. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def run_lodo():
    ...
    print(
        f"\n[GLOBAL OOF - RIDGE_D5_TDN_CLIM_NOADF] "
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_TDN_clim_ridge_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_TDN_clim_ridge_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (Ridge D5 TDN CLIM, sem ADF/TDN_NDF/CP) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (Ridge D5 TDN CLIM, sem ADF/TDN_NDF/CP) salvas em: {oof_path}")
