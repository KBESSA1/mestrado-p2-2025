...
# Notas: Script de experimento LODO para CP (proteína bruta), usando Kolmogorov-Arnold Network (KAN), janela Δ5 dias (D5), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data():
    ...
    # Features numéricas, removendo clima e alvo
    num_cols = df.select_dtypes(include=["number"]).columns.tolist()
    feature_cols = [
        c for c in num_cols
        if c not in CLIMATE_COLS and c != TARGET_COL
    ]
...
def run_lodo_kan():
    ...
    for fold_idx, date in enumerate(unique_dates, start=1):
        ...
        # KAN relativamente forte, mas razoável
        model = KAN(
        ...
        # sklearn velho: não tem squared=False -> calculamos RMSE na mão
        mse = mean_squared_error(y_true, y_pred)
        rmse = np.sqrt(mse)
        mae = mean_absolute_error(y_true, y_pred)

        print(f"[FOLD {fold_idx:02d}] Date={date.date()} | R2={r2:.3f} | RMSE={rmse:.3f} | MAE={mae:.3f}")
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_kan_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_kan_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (KAN D5 CP noclim) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (KAN D5 CP noclim) salvas em: {oof_path}")
