...
# Notas: Script de experimento LODO para CP (proteína bruta), usando Kolmogorov-Arnold Network (KAN), janela Δ5 dias (D5), com clima acoplado ao S2. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data():
    ...
    # Features numéricas, AGORA COM clima (CLIM): tiramos só o alvo
    num_cols = df.select_dtypes(include=["number"]).columns.tolist()
    feature_cols = [c for c in num_cols if c != TARGET_COL]

    print("[INFO] Colunas de clima usadas (D5 CLIM):", CLIMATE_COLS)
    print(f"[INFO] Nº de features numéricas (com clima): {len(feature_cols)}")
...
def run_lodo_kan():
    ...
    for fold_idx, date in enumerate(unique_dates, start=1):
        ...
        # KAN (mesma configuração do noclim)
        model = KAN(
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_clim_kan_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_clim_kan_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (KAN D5 CP CLIM) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (KAN D5 CP CLIM) salvas em: {oof_path}")
