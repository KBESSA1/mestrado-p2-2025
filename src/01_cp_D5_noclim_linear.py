...
# Notas: Script de experimento LODO para CP (proteína bruta), usando Regressão Linear, janela Δ5 dias (D5), apenas bandas/índices do S2, sem clima. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def prepare_data():
    ...
    # Filtro D5
    print(f"[INFO] Filtrando linhas com Delta_days <= {DELTA_MAX_DAYS}...")
    df = df[df["Delta_days"] <= DELTA_MAX_DAYS].copy()
    print(f"[INFO] Nº de amostras após filtro D5: {len(df)}")

    # Vamos ignorar clima nesse script (noclim)
    ...
    drop_cols += clima_presentes  # tirar clima
    # Também tiramos os IDs/datas numéricos, se existirem como número
    for extra in ["Sample", "Sub-Sample"]:
        if extra in num_cols:
            drop_cols.append(extra)

    feature_cols = [c for c in num_cols if c not in drop_cols]

    print(f"[INFO] Nº de features numéricas (sem clima): {len(feature_cols)}")
...
def run_lodo():
    ...
    print("[INFO] Iniciando LODO por Date (Linear D5, noclim)...")
    ...
    for i, d in enumerate(unique_dates, start=1):
        ...
        rmse = float(np.sqrt(mse))  # sem squared=False
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_linear_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D5_CP_noclim_linear_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (Linear D5 noclim) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (Linear D5 noclim) salvas em: {oof_path}")
