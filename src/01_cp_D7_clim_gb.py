...
# Notas: Script de experimento LODO para CP (proteína bruta), usando Gradient Boosting (árvore), janela Δ7 dias (D7), com clima acoplado ao S2. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def main():
    ...
    # --- features numéricas (inclui clima, se existir) ---
    numeric_cols = df_d7.select_dtypes(include=[np.number]).columns.tolist()
    drop_cols = {TARGET_COL, "Delta_days"}  # Delta_days só para o filtro
    ...
    print("[INFO] Iniciando LODO por Date (GB CLIM)...")
    ...
    print(f"\n[GLOBAL OOF - GB_CLIM] R2={r2_global:.3f} | RMSE={rmse_global:.3f} | MAE={mae_global:.3f}\n")

    # --- salvar métricas ---
    metrics_path = os.path.join(
        OUT_DIR,
        "baseline_experimento_UFMS_D7_CP_clim_gb_metrics.csv",
    )
    ...
    print(f"[OK] Métricas por fold + GLOBAL (GB CLIM) salvas em: {metrics_path}")

    # --- salvar OOF ---
    oof_path = os.path.join(
        OUT_DIR,
        "baseline_experimento_UFMS_D7_CP_clim_gb_oof.csv",
    )
    oof_df = pd.DataFrame({
    ...
    print(f"[OK] Predições OOF (GB CLIM) salvas em: {oof_path}")
