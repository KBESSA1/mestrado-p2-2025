...
# Notas: Script de experimento LODO para TDN_based_ADF (digestibilidade estrutural), usando XGBoost (árvore), janela Δ7 dias (D7), com clima acoplado ao S2. Faz parte do painel de baselines oficiais do mestrado (UFMS).
...
def run_lodo():
    ...
    for i, d in enumerate(unique_dates, start=1):
        ...
        # Escalonamento (consistência com os outros scripts)
    ...
    print(
        f"\n[GLOBAL OOF - XGB_D7_TDN_CLIM_NOADF] "
    ...
    metrics_path = OUT_DIR / "baseline_experimento_UFMS_D7_TDN_clim_xgb_metrics.csv"
    oof_path = OUT_DIR / "baseline_experimento_UFMS_D7_TDN_clim_xgb_oof.csv"

    metrics_df.to_csv(metrics_path, index=False)
    oof_df.to_csv(oof_path, index=False)

    print(f"[OK] Métricas por fold + GLOBAL (XGB D7 TDN CLIM, sem ADF/TDN_NDF/CP) salvas em: {metrics_path}")
    print(f"[OK] Predições OOF (XGB D7 TDN CLIM, sem ADF/TDN_NDF/CP) salvas em: {oof_path}")
